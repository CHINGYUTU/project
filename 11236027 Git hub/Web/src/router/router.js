...
router.beforeEach((to, from, next) => {
    ...
    if (isAuthenticated && (to.path === '/login' || to.path === '/register' || to.path === '/resPwd')) {
        next({ path: '/' });
    }else if (isAuthenticated && to.meta.Role && to.meta.Role !== userRole) { // 如果使用者已登入但角色不匹配，重定向到首頁
        next({ path: '/' });
    }else if (!isAuthenticated && to.matched.some(record => record.meta.requiresAuth)) { // 如果未登入且路由需要身份驗證，重定向到登入頁
        next({ path: '/login' });
    }else {// 否則，繼續路由跳轉
        next();
    }
