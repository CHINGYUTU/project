<template>
  <div class="flex flex-col justify-center items-center p-6 space-y-6">
    <div class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
      ...
      <form class="flex items-center w-full sm:w-auto border border-gray-300 bg-gray-50 rounded-full shadow-md px-3 py-2">      
        ...
        <input 
          type="text" 
          placeholder="輸入道路名稱" 
          class="ml-3 w-full sm:w-64 bg-transparent text-gray-700 placeholder-gray-500 focus:outline-none focus:ring-0"
          v-model="selectAddress"
          @input="searchCriteriaEvent"
        />
      </form>
    </div>
    <table class="w-full sm:w-auto text-sm text-left rtl:text-right text-gray-500 ">
      <tbody>
        <tr class="bg-white border-b " v-if="selectCity != 'None'">
            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap ">
                地區
            </th>
            <td class="px-6 py-4">
              
              <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2 mt-4">
                <label class="flex items-center space-x-2 mb-2">
                  <input type="radio" value="不限" v-model="selectArea" @change="searchCriteriaEvent" class="" />
                ...
                <label v-for="area in areas[selectCity]" :key="area" class="flex items-center space-x-2 mb-2">
                  <input type="radio" :value="area" name="area" class="form-radio text-blue-600" @change="searchCriteriaEvent" v-model="selectArea">
                  {{ area }}
                </label>
              </div>
            </td>
              ...
              <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2 mt-4">
                <label class="cursor-pointer">
                  <input
                    ...
                    @change="searchCriteriaEvent"
                    class="mr-1"
                  />
                  <span :class="{'text-orange-500': selectedPrice === '不限', 'text-gray-700': selectedPrice !== '不限'}">不限</span>
                </label>
                <label class="cursor-pointer" v-for="(price, index) in priceOptions" :key="index">
                  <input
                    type="radio"
                    :value="price"
                    v-model="selectedPrice"
        ...
        <tr class="bg-white border-b ">
          ...
          <td class="px-6 py-4">
            <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2 mt-4">
              ...
              <label class="cursor-pointer" v-for="(size, index) in sizeOptions" :key="index">
                <input
                  type="radio"
                  :value="size"
                  v-model="selectSize"
