  ...
  <div class="flex flex-col w-2/3 items-center justify-center space-y-4 rounded-lg">
    ...
    <div class="w-2/3" v-else>
      <p class="mt-4 text-xl flex items-center justify-center">
        第一張圖片為封面圖 已上傳
        <span :class="{ 'text-red-500': imageFiles.length === 9, 'text-green-500': imageFiles.length < 9 }"> {{ imageURL.length }}/9 </span>
        張照片到伺服器
      </p>
      <label
        for="dropzone-file"
      ...
      >
        <div class="flex flex-col items-center justify-center pt-5 pb-6">
          ...
          <p class="mb-2 text-sm text-gray-500"><span class="font-semibold">按此上傳</span>或拖動上傳</p>
          <p class="text-xs text-gray-500">PNG, JPG</p>
        </div>
        <input id="dropzone-file" type="file" class="hidden" accept="image/*" multiple @change="onFileChange" />
      </label>

      <!-- 图片预览区域 -->
      <div class="grid grid-cols-3 gap-4 mt-6">
        <!-- 增加 gap -->
        <div v-for="(image, index) in imagePreviews" :key="index" class="relative w-48 h-48 border border-gray-200 rounded-lg overflow-hidden">
          <img :src="image" alt="Image Preview" class="w-full h-full" />
          <!-- 删除按钮 -->
          <button
    ...
    <div class="w-2/3 grid grid-cols-3 gap-4 mt-2" v-if="this.imageURL">
      <!-- 增加 gap -->
      <div v-for="(image, index) in imageURL" :key="index" class="relative w-48 h-48 border border-gray-200 rounded-lg overflow-hidden">
        <img :src="image" alt="Image Preview" class="w-full h-full" />
      </div>
    </div>
  ...
  methods: {
    ...
    async UploadPic() {
      ...
      for (let i = 0; i < totalImages; i++) {
        ...
        formData.append("token", "cbdc33bdaada0743d7cb63331e35451a");
