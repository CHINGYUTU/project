<template>
  <div class="mx-auto p-6 bg-white rounded-lg shadow-lg md:max-w-4xl max-w-md">
    ...
    <div class="flex justify-between items-center">
      ...
      <button @click="changelike()" v-if="showLikeBtn">
        <i class="fa-regular fa-heart" v-if="!like"></i><img src="/Like icon.png" class="w-4" v-else />
      </button>
    </div>
    ...
    <morepic
      v-if="
        houseInfo.Pic1 || houseInfo.Pic2 || houseInfo.Pic3 || houseInfo.Pic4 || houseInfo.Pic5 || houseInfo.Pic6 || houseInfo.Pic7 || houseInfo.Pic8
      "
      :Pic1="houseInfo.Pic1"
      :Pic2="houseInfo.Pic2"
      :Pic3="houseInfo.Pic3"
    ...
    <div class="flex items-center justify-between w-full">
      <div class="flex items-center">
        <router-link>
          <img :src="houseInfo.Headshot" alt="User Avatar" class="w-16 h-16 rounded-full border-2 border-gray-300 shadow-md" />
        </router-link>
      ...
      <evaluate class="flex items-end justify-end" :LID="houseInfo.LandlordID"></evaluate>
    </div>

    <div class="my-4 border-b border-gray-300"></div>

    <div class="flex items-center">
      <img src="/Feedback.png" class="h-10" alt="設備圖標" />
      <p class="ml-2 text-2xl font-bold">評價</p>
      <!-- <button class="bg-blue-500 text-white px-4 py-2 rounded-full ml-auto" v-if="showLikeBtn && !filledIn">填寫評論</button> -->
    </div>
    <message v-if="showLikeBtn && !filledIn"></message>
    <feedback class="ml-2" :ID="this.$route.params.id"></feedback>
...
export default {
  ...
  methods: {
    getHouseInfo() {
      axios
        .post("/api/getSpecifyHouseInfo", { ID: this.id })
        .then((res) => {
    ...
    initLike() {
      if (localStorage.getItem("session")) {
        const role = JSON.parse(localStorage.getItem("session")).Role;
        if (JSON.parse(role === "User")) {
          this.showLikeBtn = true;
          axios
            .post("/api/checklike", { UserID: JSON.parse(localStorage.getItem("session")).ID, HouseID: this.id })
            .then((res) => {
              this.like = res.data.liked;
            })
            .catch((err) => {});
          axios
            .post("/api/checkfeedback", { UserID: JSON.parse(localStorage.getItem("session")).ID, HouseID: this.id })
            .then((res) => {
              this.filledIn = res.data.filledIn;
            })
            .catch((err) => {});
        } else {
          this.showLikeBtn = false;
        }
      }
    },
  },
